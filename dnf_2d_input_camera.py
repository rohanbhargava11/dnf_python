...
while True:
    ...
    #input_image=cv2.imread('/home/rohan/Documents/dnf_python/test1.jpg',0)
    input_image=cv2.resize(im,(101,101))
    input_image1=cv2.resize(im,(101,101))
    for i in range (101):
        for j in range (101):
            if input_image[i][j]>100:
                input_image[i][j]=0
                input_image1[i][j]=255
            else:
                input_image[i][j]=1
                input_image1[i][j]=0
    #cv2.imshow('video_test',input_image)
    #cv2.waitKey(10)
    I_ext=input_image
    #cv2.imshow('video test',input_image1)
    #cv2.waitKey(5)#/np.max(input_image)
#cv2.imshow('test',input_image)
#I_ext[int(nn/2-np.floor(nn/20)):int(nn/2+np.floor(nn/20))+1,
